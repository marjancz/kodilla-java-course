...
public class PlayerMoveExtended {
    ...
    public Figure getPlayerMove() throws NullPointerException {
        System.out.println("\nPlease type your choice: Scissors(1), Rock(2), Paper(3), Lizard(4), Spock(5) " +
                "\nor " +
                "\nnew game(n) " +
                "\nexit game(x)");
        playerMove = sc.next();
        ...
        while (!end) {
            if (!(playerMove.equals("1") || playerMove.equals("2") || playerMove.equals("3") ||
                    playerMove.equals("4") || playerMove.equals("5") || playerMove.equals("n") ||
                    playerMove.equals("x"))) {
                System.out.println("Wrong!!! You can only choose from " +
                        " \nScissors(1) ,Rock(2),Paper(3), Lizard(4), Spock(5), new game(n), exit game(x)");
                getPlayerMove();
        ...
        switch (playerMove) {
            ...
            case "4":
                playerMove = FigureNames.LIZARD;
                return new Lizard("Lizard", FigureNames.lizardWinWith());
            case "5":
                playerMove = FigureNames.SPOCK;
                return new Spock("Spock", FigureNames.spockWinWith());
            default:
                return null;
        }
    }
